...
const EventSpecificPage = async ({ params: { eventId } }: EventSpecificPageProps) => {
    ...
    return (
        <PageContentWrapper>
            <div className={"w-5/6 h-fit max-w-screen-xl flex flex-col-reverse grid-cols-1 md:grid md:grid-rows-1 md:grid-cols-[2fr_1fr]"}>
                <SingleEventTitleBar event={event} />
                {event && <SingleEventTeamLogoBox teams={event?.participants} />}
            </div>
            <div className={"w-full h-fit md:w-5/6 min-h-[40vh] max-w-screen-xl flex flex-col justify-start items-center gap-12 md:gap-4 md:flex-row my-8"}>
